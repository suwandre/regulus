...
- **Chassis / EC controls (fan curves, OASIS watercooler, MUX switch, XMG-specific unlocks) — excluded from M1.** All require reverse-engineering (unbounded work); isolated to later milestones (§10) so they cannot block a shippable core.
...
│   (chassis/EC + OASIS → roadmap, M3/M5)       │
└─────────────────────────────────────────────┘
```
...
| Milestone | Scope |
|-----------|-------|
| **M1** | Modular core + HAL + capability detection + `power` + `telemetry` + profiles + reapply + both UIs. v1 backend: Windows·x86·AMD·NVIDIA·XMG-Tongfang, **CPU/GPU power + telemetry only — no chassis/EC.** **The runnable app.** |
| **M2** | GPU overclock/undervolt via NVAPI (clock/voltage curve). |
| **M3** | Display module: auto refresh-rate, brightness; MUX / dGPU-only / Optimus switching. Chassis/EC: fan control + XMG-specific toggles/unlocks. (All EC reverse-engineering gated.) |
...
- Each task also carries a **`model·effort`** tag: Opus 4.8 · xhigh/max for architecture, `unsafe`/FFI, ring0, and RE work; Sonnet 4.6 (or Opus · medium) for tested pure logic and UI wiring; Sonnet 4.6 · low/medium for boilerplate. `max` reserved for genuinely hard isolated problems.
...
- Detail lives in the implementation plan, not this spec: every task gets a concrete deliverable and **testable acceptance criteria** (a pass/fail test where possible). Ralph's convergence depends on this — vague criteria cause loops or false "done". The plan is authored, not self-generated by the agents.
